...
| 36   | ```python main.py --experiment=experiment2 --config=configs/paper_2.yml --dataset="small" --config.data.plt_fig=True --sample_inputs --split="random" --watch=accuracy``` | Same experiment as #10, but with a random split instead of a temporal split. The goal is to identify the effect of the method of splitting on the evaluation performances. |
